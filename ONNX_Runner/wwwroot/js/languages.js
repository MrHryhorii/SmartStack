export const SUPPORTED_LANGUAGES = [
    { code: "auto", name: "Auto-detect", note: "Uses the server model language and automatic language-detection settings." },
    { code: "af", name: "Afrikaans" },
    { code: "sq", name: "Albanian" },
    { code: "am", name: "Amharic" },
    { code: "grc", name: "Ancient Greek" },
    { code: "ar", name: "Arabic", note: "Fully diacritized text only." },
    { code: "an", name: "Aragonese" },
    { code: "hy", name: "Armenian" },
    { code: "as", name: "Assamese" },
    { code: "az", name: "Azerbaijani" },
    { code: "ba", name: "Bashkir" },
    { code: "eu", name: "Basque" },
    { code: "be", name: "Belarusian" },
    { code: "bn", name: "Bengali" },
    { code: "bpy", name: "Bishnupriya Manipuri" },
    { code: "bs", name: "Bosnian" },
    { code: "bg", name: "Bulgarian" },
    { code: "my", name: "Burmese" },
    { code: "yue", name: "Cantonese Chinese" },
    { code: "ca", name: "Catalan" },
    { code: "chr", name: "Cherokee", note: "Annotated dictionary text only." },
    { code: "cv", name: "Chuvash" },
    { code: "crh", name: "Crimean Tatar" },
    { code: "hr", name: "Croatian" },
    { code: "cs", name: "Czech" },
    { code: "da", name: "Danish" },
    { code: "nl", name: "Dutch" },
    { code: "en", name: "English" },
    { code: "eo", name: "Esperanto" },
    { code: "et", name: "Estonian" },
    { code: "fo", name: "Faroese" },
    { code: "fi", name: "Finnish" },
    { code: "fr", name: "French" },
    { code: "ka", name: "Georgian" },
    { code: "de", name: "German" },
    { code: "el", name: "Greek" },
    { code: "kl", name: "Greenlandic" },
    { code: "gn", name: "Guarani" },
    { code: "gu", name: "Gujarati" },
    { code: "ht", name: "Haitian Creole" },
    { code: "hak", name: "Hakka Chinese" },
    { code: "haw", name: "Hawaiian" },
    { code: "he", name: "Hebrew" },
    { code: "hi", name: "Hindi" },
    { code: "hu", name: "Hungarian" },
    { code: "is", name: "Icelandic" },
    { code: "io", name: "Ido" },
    { code: "id", name: "Indonesian" },
    { code: "ia", name: "Interlingua" },
    { code: "ga", name: "Irish" },
    { code: "it", name: "Italian" },
    { code: "ja", name: "Japanese", note: "Hiragana/Katakana only." },
    { code: "kn", name: "Kannada" },
    { code: "kaa", name: "Karakalpak" },
    { code: "kk", name: "Kazakh" },
    { code: "quc", name: "K'iche'" },
    { code: "piqd", name: "Klingon" },
    { code: "kok", name: "Konkani" },
    { code: "ko", name: "Korean" },
    { code: "ku", name: "Kurdish" },
    { code: "ky", name: "Kyrgyz" },
    { code: "qdb", name: "Lang Belta" },
    { code: "ltg", name: "Latgalian" },
    { code: "la", name: "Latin" },
    { code: "lv", name: "Latvian" },
    { code: "lfn", name: "Lingua Franca Nova" },
    { code: "lt", name: "Lithuanian" },
    { code: "jbo", name: "Lojban" },
    { code: "smj", name: "Lule Saami" },
    { code: "lb", name: "Luxembourgish" },
    { code: "mk", name: "Macedonian" },
    { code: "ms", name: "Malay" },
    { code: "ml", name: "Malayalam" },
    { code: "mt", name: "Maltese" },
    { code: "cmn", name: "Mandarin Chinese" },
    { code: "mi", name: "Māori" },
    { code: "mr", name: "Marathi" },
    { code: "nci", name: "Nahuatl" },
    { code: "ne", name: "Nepali" },
    { code: "nog", name: "Nogai" },
    { code: "nb", name: "Norwegian Bokmål" },
    { code: "or", name: "Oriya" },
    { code: "om", name: "Oromo" },
    { code: "pap", name: "Papiamento" },
    { code: "fa", name: "Persian" },
    { code: "pl", name: "Polish" },
    { code: "pt", name: "Portuguese" },
    { code: "pa", name: "Punjabi" },
    { code: "py", name: "Pyash" },
    { code: "qu", name: "Quechua" },
    { code: "qya", name: "Quenya" },
    { code: "ro", name: "Romanian" },
    { code: "ru", name: "Russian" },
    { code: "gd", name: "Scottish Gaelic" },
    { code: "sr", name: "Serbian" },
    { code: "tn", name: "Setswana" },
    { code: "shn", name: "Shan" },
    { code: "sjn", name: "Sindarin" },
    { code: "sd", name: "Sindhi" },
    { code: "si", name: "Sinhala" },
    { code: "sk", name: "Slovak" },
    { code: "sl", name: "Slovenian" },
    { code: "es", name: "Spanish" },
    { code: "sw", name: "Swahili" },
    { code: "sv", name: "Swedish" },
    { code: "ta", name: "Tamil" },
    { code: "tt", name: "Tatar" },
    { code: "te", name: "Telugu" },
    { code: "th", name: "Thai" },
    { code: "ti", name: "Tigrinya" },
    { code: "mto", name: "Totontepec Mixe" },
    { code: "tr", name: "Turkish" },
    { code: "tk", name: "Turkmen" },
    { code: "uk", name: "Ukrainian" },
    { code: "ur", name: "Urdu" },
    { code: "ug", name: "Uyghur" },
    { code: "uz", name: "Uzbek" },
    { code: "vi", name: "Vietnamese" },
    { code: "cy", name: "Welsh" },
    { code: "hyw", name: "Western Armenian" },
    { code: "xex", name: "Xextan" },
];