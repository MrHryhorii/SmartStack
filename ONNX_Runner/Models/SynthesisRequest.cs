...
public class SynthesisRequest
{
    ...
    /// <summary>
    /// Controls response framing independently from delivery timing.
    /// Audio preserves the existing binary/JSON response path; SSE wraps output in
    /// OpenAI-compatible Server-Sent Events.
    /// </summary>
    public SpeechStreamFormat StreamFormat { get; set; } = SpeechStreamFormat.Audio;
    /// <summary>
    /// Controls whether output is delivered incrementally while generation is running.
    /// False buffers the complete result before delivery. Formats such as WAV may still
    /// require buffering even when this value resolves to true.
    /// </summary>
